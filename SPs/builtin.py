...
builtInSPsList = [
           ...
           #[ "make_dag",dagDir.makeDAGSP ],
           #[ "make_dag2",DAGsampler.makeDAGsamplerSP ],
           ["edge", typed_nr(edge.Edge(), [v.AnyType(),v.IntegerType(),v.IntegerType(),v.ProbabilityType(),v.AnyType(),v.AtomType(),v.AtomType()], v.BoolType(), min_req_args=3)],
           ["edge2", typed_nr(edgePrior.EdgePrior(), [v.IntegerType(),v.IntegerType(),v.ProbabilityType(),v.HomogeneousArrayType(v.CountType()),v.AtomType(),v.AtomType()], v.BoolType(), min_req_args=2)],
          
           [ "dir_dag",typed_nr(dirichlet.SymmetricDirichletDAGOutputPSP(), [v.PositiveType(),v.HomogeneousArrayType(v.HomogeneousArrayType(v.BoolType())), v.HomogeneousArrayType(v.IntegerType()),v.AnyType(),v.BoolType()],v.MatrixType(),min_req_args=4) ], # two d array for dag,ranges out dataVector
           ["make_gp_grammar", typed_nr(GPgrammar.MakeGPGrammarOutputPSP(), [v.HomogeneousArrayType(v.AnyType()),v.HomogeneousArrayType(v.AnyType()),v.HomogeneousArrayType(v.AnyType()),v.HomogeneousArrayType(v.AnyType()),v.ProbabilityType(),v.ProbabilityType()], v.AnyType(), min_req_args=0)],
           ["gp_grammar", typed_nr(grammar2.Grammar(), [v.HomogeneousArrayType(v.HomogeneousArrayType(v.AnyType())),v.SimplexType(),v.ProbabilityType()], v.AnyType(), min_req_args=0)],
           ["gp_grammar2", typed_nr(grammar3.Grammar(), [v.HomogeneousArrayType(v.HomogeneousArrayType(v.AnyType())),v.SimplexType(),v.CountType(),v.CountType()], v.AnyType(), min_req_args=0)],
           [ "in_order", typed_nr(order.Order(), [v.HomogeneousArrayType(v.NumberType()),v.AnyType(),v.AnyType()], v.BoolType())],
           [ "covfunc_interpreter",typed_nr(grammarinterpreter.GrammarInterpreter(), [v.AnyType()], v.AnyType())],
