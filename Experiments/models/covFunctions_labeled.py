...
def makeConst(sf,label=[]):
  return VentureFunction(const(sf), sp_type=covfunctionType,name="C",derivatives={0:const_der(sf)},parameter=[sf],label_list=[v.VentureInteger(label)])
...
def makeSquaredExponential(sf, l,label=[]):
  return VentureFunction(squared_exponential(sf, l), sp_type=squaredExponentialType,derivatives={0:squared_exponential_der_l(sf,l),1:squared_exponential_der_sf(sf,l)},name="SE",parameter=[sf,l],label_list=[v.VentureInteger(label)])
...
def makePeriodic(l,p,sf,label=[]):
  return VentureFunction(periodic(l,p,sf), sp_type=covfunctionType,derivatives={0:periodic_der_l(l,p,sf),1:periodic_der_p(l,p,sf),2:periodic_der_sf(l,p,sf)},name="PER",parameter=[l,p,sf],label_list=[v.VentureInteger(label)])
...
def makeLinear(sf,label=[]):
  return VentureFunction(linear(sf), sp_type=covfunctionType,derivatives={0:linear_der_sf(sf)},name="LIN",parameter=[sf],label_list=[v.VentureInteger(label)])
...
def makeNoise(s,label=[]):
  return VentureFunction(noise(s), sp_type=covfunctionType,derivatives={0:noise_der(s)},name="WN",parameter=[s],label_list=[v.VentureInteger(label)])
...
def makeRQ(l,sf,alpha,label=[]):
  return VentureFunction(rationalQuadratic(l,sf,alpha), sp_type=covfunctionType,derivatives={0:rationalQuadratic_der_l(l,sf,alpha),1:rationalQuadratic_der_sf2(l,sf,alpha),2:rationalQuadratic_der_alpha(l,sf,alpha)},name="RQ",parameter=[l,sf,alpha],label_list=[v.VentureInteger(label)])
...
def makeLiftedAdd(op):
  ...
  def wrapped(f1, f2):
    ...
    return VentureFunction(lifted_op(f1, f2), sp_type=sp_type,derivatives=der,name=f1.stuff['name']+"+"+f2.stuff['name'],label_list=f1.stuff['label_list']+f2.stuff['label_list'])
...
def makeLiftedMult(op):
  ...
  def wrapped(f1, f2):
    ...
    return VentureFunction(lifted_op(f1,f2), sp_type=sp_type,derivatives=der,name=f1.stuff['name']+"x"+f2.stuff['name'],label_list=f1.stuff['label_list']+f2.stuff['label_list'])
  return VentureFunction(wrapped, sp_type=liftedBinaryType)
