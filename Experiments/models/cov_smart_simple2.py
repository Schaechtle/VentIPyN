...
class Grammar_Venture_GP_smart_simple2(Venture_GP_model):
    def __init__(self):
    ...
    def make_gp(self, ripl):
        ...
        ripl.assume('b',' (tag (quote 1) 0 (uniform_continuous  0.01 10))')
        ripl.assume('c',' (tag (quote 1 ) 1 (uniform_continuous  0.01 10))')
        ripl.assume('d','(tag (quote 1 ) 2 (uniform_continuous  0.01 10))')

        ripl.assume('lin', "(apply_function make_linear a  0 )")
        ripl.assume('per', "(apply_function make_periodic b c d 1 ) ")
